...
var sqlite3 = require('sqlite3').verbose()
	...
	, db = module.exports = (function () {
		var archDatabase = path.resolve(".\\basededatos\\CotizacionEspirituosa.db")
			, database = new pool({
				user: 'Cotizacion_Espirituosa',
				host: 'localhost',
				database: 'Cotizacion_Espirituosa',
				password: 'volcan',
				port: 5432,
			})
			, arrTablas = [{
                                            ...
                                            );'},
			{
				...
				, CreacionTabla: 'CREATE TABLE IF NOT EXISTS Grupos_Bebida (	\
                                            ...
                                            ,   Tipos_Genericos NUMERIC(1,0) NOT NULL \n\
				...
				, {
					Tabla: 'Bebidas_Sesiones', Indice: 'CAST(Id_Sesion AS VARCHAR(5))+CAST(Id_Bebida AS VARCHAR(5))'
				...
				, {
					Tabla: 'Precios', Indice: 'CAST(Id_G_Bebida AS VARCHAR(5))+CAST(Id_T_Servicio AS VARCHAR(5))'
				...
				, {
					Tabla: 'Cotizacion', Indice: 'CAST(Id_Bebida AS VARCHAR(5))+CAST(Id_T_Servicio AS VARCHAR(5))'
				...
				, {
					Tabla: 'Perfiles_Permisos', Indice: 'CAST(Id_Perfil AS VARCHAR(5))+CAST(Id_Permiso AS VARCHAR(5))'
			...
			, arrValoresxDefecto = [
				...
				, "INSERT INTO Grupos_Bebida VALUES(-1, 'Default', 1, 0, 0);"
			...
			, InsertarValoresxDefecto = function () {
				...
				ExecSQL(arrValoresxDefecto.join(" "));
			}
			...
			, ExecSQL = function (/* Comando SQL */cmdSQL, /* evento resultante */ EvObtenerResultados, _self) {
				...
				if (cmdSQL.length === 1) {
					database.query(cmdSQL[0], oResultado.fnResultado);
					database.end();
				} else {
					var cmd;
					for (cmd of cmdSQL) {
						database.query(cmd + ";", (error, _results) => {
